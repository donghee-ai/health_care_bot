...
## 7. 왜 컴포넌트별 테마 분기가 아니라 CSS 변수인가

- 모든 색상을 처음부터 `:root`의 CSS custom property로 뽑아뒀기 때문에,
  "레퍼런스 이미지에 가까운 톤도 보고 싶다"는 요청이 왔을 때
  `:root[data-theme='reference'] { --primary: ...; }` 블록 하나로 앱
  전체에 반영됐다 — 컴포넌트 JSX/CSS를 단 한 줄도 안 건드렸다.
  (`index.css`의 `[data-theme='reference']` 섹션, `hooks/useTheme.ts`)
- 두 테마를 별도 브랜치/폴더로 나눴다면 이후 기능 추가마다 두 번 고쳐야
  했을 것 — 변수 치환 방식은 유지보수 비용이 사실상 0에 가깝다.

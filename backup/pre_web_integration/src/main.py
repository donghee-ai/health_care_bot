...
def parse_args():
    ...
    ap.add_argument("--serial", default="",
                    help="Arduino 시리얼 포트 (예: /dev/ttyACM0). 미지정 시 PTZ 비활성")
    ap.add_argument("--baudrate", type=int, default=115200)
    ap.add_argument("--frame-out-margin", type=float, default=0.15)
    ap.add_argument("--frame-out-grace", type=int, default=15)
    ap.add_argument("--pan-step-deg", type=float, default=8.0)
    ap.add_argument("--tilt-step-deg", type=float, default=5.0)
    ap.add_argument("--ptz-cooldown-ms", type=float, default=400.0)
...
def main():
    ...
    print(f"  ptz      : serial={args.serial or '(disabled)'} margin={args.frame_out_margin} grace={args.frame_out_grace}f")
    if args.serve:
    ...
    ptz = PTZController(
        ...
        frame_out_margin=args.frame_out_margin,
        frame_out_grace=args.frame_out_grace,
        pan_step_deg=args.pan_step_deg,
    ...
    try:
        while True:
            ...
            # PTZ 추적점: 스쿼트는 무릎 가중 추적(SQUAT_TRACK_WEIGHTS)으로 무릎이
            # 화면 안에 우선 유지되게 하고, 그 외(pushup/unknown)는 기존 상반신 중심
            if exercise == "squat":
            ...
            ptz_cmd = ptz.update(x_norm, y_norm, now_ms)

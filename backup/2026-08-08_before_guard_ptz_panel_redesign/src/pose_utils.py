...
def _mean_center(kp, names, frame_h, frame_w, conf_th):
    """지정 keypoint 중 conf 통과한 것들의 평균 → 정규화 (x_norm, y_norm)."""
...
def knee_center_normalized(kp, frame_h, frame_w, conf_th=0.3, require_both=True):
    """두 무릎 중점 → 정규화 (x_norm, y_norm).
    ...
    if require_both and not (kp[li, 2] >= conf_th and kp[ri, 2] >= conf_th):
        return None, None
    return _mean_center(kp, ("left_knee", "right_knee"), frame_h, frame_w, conf_th)


def hip_center_normalized(kp, frame_h, frame_w, conf_th=0.3):
    """엉덩이 중점 → 정규화. 무릎이 한쪽만 보일 때의 폴백 (몸통 중심선이라 좌우 쏠림 적음)."""

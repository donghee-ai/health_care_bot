...
### 7-1. 이 리포는 공개다 — 문서에 적지 않는 것

GitHub 공개 저장소이므로 아래는 **값을 적지 말고 "확인하는 방법"만 적는다.**

| 적지 않는 것 | 대신 |
|---|---|
| Tailscale Funnel 주소 · Tailscale IP | `tailscale status` / `tailscale ip -4`로 확인 |
| adb serial 등 기기 고유 식별자 | `adb devices`로 확인 |
| 실제 운영자 PIN | `src/app_state.py::_operator_pin` 위치만 언급 |
| Wi-Fi SSID·비밀번호, MAC 주소 | 적지 않는다 |
| 사람이 찍힌 사진 (`captures/`) | `.gitignore` 대상. 커밋 금지 |

사설 IP(`192.168.x.x`)는 외부에서 라우팅되지 않아 예외로 둔다 — 런북에서 실용성이 크다.

> **이미 커밋한 뒤에 발견하면 파일만 지워서는 안 된다.** 공개 리포는 과거 커밋에서 그대로
> 받아갈 수 있으므로 `git filter-repo`로 히스토리에서 지우고 force push해야 한다. 이때
> **파일이 이동한 적이 있으면 옛 경로도 함께 지워야 한다** — 경로 하나만 지우면 이전
> 경로의 사본이 그대로 남는다(2026-09-08 실제로 밟음).

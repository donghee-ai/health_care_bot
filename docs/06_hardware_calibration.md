...
| **기둥** | 두 통을 잇는 축. pitch 관절이 붙고, 데이지체인 케이블이 이 안으로 지난다 |
| **베이스**(아래쪽 육각 통) | **yaw(좌우) 서보 ID=1** + UNO Q + 셀프파워 허브 + Bus Servo Adapter |

그래서 **yaw는 헤드+기둥 전체를 돌리고, pitch는 헤드만 끄덕인다.**

서보 버스(데이지체인) 케이블은 **기둥 안으로 지난다.** 실물 사진
([`assets/robot.jpg`](assets/robot.jpg))에서 헤드-베이스 사이에 바깥으로 나온 루프가
보이는데, 이건 배선이 새는 게 아니라 **회전 여유를 주려고 일부러 밖으로 빼둔 여유분**이다.

> **이 여유분을 정리하거나 타이로 묶지 말 것.** 여유가 없으면 yaw가 돌 때 케이블이
> 당겨진다.
